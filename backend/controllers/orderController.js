...
// Create preorder (for PreOrder page)
const createPreorder = async (req, res) => {
  try {
    ...
    const order = new Order({
      ...
      table: orderType === 'dine-in' ? table : `${orderType.toUpperCase()}-ORDER`,
      items,
      totalAmount,
      notes: notes || '',
      status: 'pending'
    ...
    // Preorders are scheduled orders - they don't go to kitchen dashboard immediately
    // Only admin dashboard should handle preorders for scheduling and management
    // No real-time kitchen notifications for preorders
